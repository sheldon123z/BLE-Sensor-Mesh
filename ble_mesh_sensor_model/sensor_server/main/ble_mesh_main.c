...
#define SENSOR_PROPERTY_ID_1        0x005B  /* Present Object Temperature */
...
#define BH1750_SENSOR_ADDR 0x10 
...
static void send_sensor_descriptor_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
...
static void send_sensor_cadence_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    esp_err_t err;
...
static void send_sensor_settings_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    esp_err_t err;
...
static void send_sensor_setting_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
...
static esp_err_t operate_i2c_master_temp_sensor(i2c_port_t i2c_num, uint8_t sensor_data[])
{
    int ret;
    ...
    for(int i = 0; i < 5; i++)
    {
       i2c_master_read_byte(cmd, &sensor_data[i], ACK_VAL);
    }
    i2c_master_read_byte(cmd, &sensor_data[5], NACK_VAL);
    i2c_master_stop(cmd);
...
static void get_i2c_temperature()
{
    uint8_t sensor_data[6];
    int ret;
    ret = operate_i2c_master_temp_sensor(I2C_MASTER_NUM, sensor_data);
    if (ret == ESP_ERR_TIMEOUT) {
        ESP_LOGE(TAG, "I2C Timeout");
    } else if (ret == ESP_OK) {
        net_buf_simple_reset(&sensor_data_0);
        net_buf_simple_add_u8(&sensor_data_0, sensor_data[0]);
        printf("sensor data[%d], %x\n", 0,sensor_data[0]);
        net_buf_simple_add_u8(&sensor_data_0, sensor_data[1]);
        printf("sensor data[%d], %x\n", 1,sensor_data[1]);
        net_buf_simple_add_u8(&sensor_data_0, sensor_data[2]);
        printf("sensor data[%d], %x\n", 2,sensor_data[2]);

        net_buf_simple_reset(&sensor_data_1);
        net_buf_simple_add_u8(&sensor_data_1, sensor_data[3]);
        printf("sensor data[%d], %x\n", 0,sensor_data[3]);
        net_buf_simple_add_u8(&sensor_data_1, sensor_data[4]);
        printf("sensor data[%d], %x\n", 1,sensor_data[4]);
        net_buf_simple_add_u8(&sensor_data_1, sensor_data[5]);
        printf("sensor data[%d], %x\n", 2,sensor_data[5]);
        

        
        float amb = (sensor_data[2] * 65536L + sensor_data[1] * 256L + sensor_data[0]) / 200.;
        float obj = (sensor_data[5] * 65536L + sensor_data[4] * 256L + sensor_data[3]) / 200.;
        printf("amb temp %02f\n",amb);
        printf("obj temp %02f\n",obj);
    } else {
...
static uint16_t get_sensor_data(esp_ble_mesh_sensor_state_t *state, uint8_t *data)
{
...
static void send_sensor_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    uint8_t *status = NULL;
    uint16_t buf_size = 0;
    uint16_t length = 0;
    ...
    if (param->value.get.sensor_data.op_en == false) {
         ...
         */
        for (i = 0; i < ARRAY_SIZE(sensor_states); i++) {
            length += get_sensor_data(&sensor_states[i], status + length);
        }
        goto send;
    }
    ...
    for (i = 0; i < ARRAY_SIZE(sensor_states); i++) {
        if (param->value.get.sensor_data.property_id == sensor_states[i].sensor_property_id) {
            length = get_sensor_data(&sensor_states[i], status);
            goto send;
        }
    }
...
static void send_sensor_column_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    uint8_t *status = NULL;
    uint16_t length = 0;
    esp_err_t err;
...
static void send_sensor_series_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    esp_err_t err;
...
//Task to show temperature locally 
// static void show_temperature(void *arg)
// {
//     uint8_t sensor_data[6];
...
void app_main(void)
{
    ...
    // xTaskCreate(show_temperature, "show_temperature_0", 1024 * 2, (void *)0, 10, NULL);
    // xTaskCreate(show_temperature, "show_temperature_1", 1024 * 2, (void *)1, 10, NULL);
}
