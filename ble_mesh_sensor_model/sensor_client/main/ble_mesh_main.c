                               ...
                               uint16_t primary_addr, uint8_t element_num, uint16_t net_idx)
{
    ...
    set_msg_common(&common, node, config_client.model, ESP_BLE_MESH_MODEL_OP_COMPOSITION_DATA_GET);
...
static void parse_node_comp_data(const uint8_t *data, uint16_t length)
{
                                              ...
                                              esp_ble_mesh_cfg_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_CFG_CLIENT_GET_STATE_EVT:
        if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_COMPOSITION_DATA_GET) {
            ...
            parse_node_comp_data(param->status_cb.comp_data_status.composition_data->data,
                param->status_cb.comp_data_status.composition_data->len);
            ...
            set_msg_common(&common, node, config_client.model, ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD);
    ...
    case ESP_BLE_MESH_CFG_CLIENT_SET_STATE_EVT:
        if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD) {
            set_msg_common(&common, node, config_client.model, ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND);
...
void send_sensor_message(uint32_t opcode)
{
    ...
    set_msg_common(&common, node, sensor_client.model, opcode);
    switch (opcode) {
...
static void sensor_timeout(uint32_t opcode)
{
    switch (opcode) {
                                              ...
                                              esp_ble_mesh_sensor_client_cb_param_t *param)
{
            ...
            if (param->status_cb.sensor_status.marshalled_sensor_data->len) {
                ...
                for (; length < param->status_cb.sensor_status.marshalled_sensor_data->len; ) {
                    ...
                    if (data_len != ESP_BLE_MESH_SENSOR_DATA_ZERO_LEN) {
                        ...
                        float temp = (*(temp_data+mpid_len+2) * 65536L + *(temp_data+ mpid_len+1) * 256L + *(temp_data + mpid_len)) / 200.;
                        if(prop_id==0x056)
                        {
                            ESP_LOGW("Ambient Temperature", "%02f Celsius",temp);
                        }
                        else
                        {
                            ESP_LOGW("Object Temperature", "%02f Celsius",temp);
                        }
                    } else {
                        length += mpid_len;
                    }
    ...
    case ESP_BLE_MESH_SENSOR_CLIENT_TIMEOUT_EVT:
        sensor_timeout(param->params->opcode);
    default:
        break;
    }
}
